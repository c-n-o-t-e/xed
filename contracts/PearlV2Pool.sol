...
contract PearlV2Pool is IPearlV2Pool {
    ...
    // accumulated rebase balance in token0/token1 units
    struct RebaseAmount {
        uint256 token0;
        uint256 token1;
    }
    ...
    /// @inheritdoc IPearlV2Pool
    RebaseAmount public override rebaseAmount;
    ...
    /// @dev Prevents calling a function from anyone except the factory address
    ...
    ///  Amounts are only calculated when liquidity is removed from the pool i.e params.liquidityDelta < 0,
    ...
    function mint(
        ...
        uint128 amount,
        bytes calldata data
    ) external override lock returns (uint256 amount0, uint256 amount1, uint128 actualLiquidity) {
        require(amount > 0);
        checkTicks(tickLower, tickUpper);
        ...
        //update reserves
        (cache.receivedAmount0, cache.receivedAmount1) = _updateReserves();
        IUniswapV3MintCallback(msg.sender).uniswapV3MintCallback(amount0, amount1, data);
        cache.receivedAmount0 = amount0 == 0 ? 0 : balance0() - cache.receivedAmount0;
        cache.receivedAmount1 = amount1 == 0 ? 0 : balance1() - cache.receivedAmount1;

        {
            actualLiquidity = amount;
            if (cache.receivedAmount0 != amount0 || cache.receivedAmount1 != amount1) {
                actualLiquidity = LiquidityAmountsV2.getLiquidityForAmounts(
                    cache.sqrtPriceX96,
                    TickMath.getSqrtRatioAtTick(tickLower),
                    TickMath.getSqrtRatioAtTick(tickUpper),
                    cache.receivedAmount0,
                    cache.receivedAmount1
                );
            }
            ...
            amount0 = cache.receivedAmount0;
            amount1 = cache.receivedAmount1;
        }
        ...
        emit Mint(msg.sender, recipient, tickLower, tickUpper, amount, amount0, amount1, actualLiquidity);
    }
    ...
    function swap(
        ...
        (amount0, amount1, state) = _calculateSwapStep(zeroForOne, amountSpecified, sqrtPriceLimitX96, slot0Start);
    ...
    function swapFeeOnTransfer(
...

        {
            ...
            if (zeroForOne) {
                ...
                (balanceBefore, balanceAfter) = (balance0Before, balance0());
            } else {
                IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(0, amountSpecified, data);
                (balanceBefore, balanceAfter) = (balance1Before, balance1());
            }

            int256 amountReceived = (balanceAfter - balanceBefore).toInt256();
            if (amountReceived < amountSpecified) amountSpecified = amountReceived;
        }
    ...
    function _calculateSwapStep(
    ...
    ) internal returns (int256 amount0, int256 amount1, SwapState memory state) {
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            (state.sqrtPriceX96, step.amountIn, step.amountOut, step.feeAmount) = SwapMathV2.computeSwapStep(
                state.sqrtPriceX96,
        ...
        if (state.tick != slot0Start.tick) {
            // notify gauge to track tick momvements for the reward
            if (gauge != address(0)) {
                // As it notifies the tick change to the gauge Contract, checking the bool value is unnecessary.
                IGaugeV2(gauge).crossTo(state.tick, zeroForOne);
            }
    ...
    function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override lock {
        ...
        //Sync reserves to account fees delta
        _syncReserves();
    ...
    /// @inheritdoc IPearlV2Pool
    function setGauge(address _gauge) external override onlyFactory {
        gauge = _gauge;
    ...
    ///@dev update reserves with the balance by keeping
    /// an account of surplus tokens from the rebasing event
    function _updateReserves() internal returns (uint256 bal0, uint256 bal1) {
        (bal0, bal1) = (balance0(), balance1());
        uint128 _liquidity = liquidity;
        if (_liquidity == 0) return (bal0, bal1);

        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        (bool hasExcessToken0, bool hasExcessToken1) = (bal0 > _reserve0, bal1 > _reserve1);

        if (hasExcessToken0 || hasExcessToken1) {
            if (hasExcessToken0) {
                rebaseAmount.token0 += (bal0 - _reserve0);
            }

            if (hasExcessToken1) {
                rebaseAmount.token1 += (bal1 - _reserve1);
            }
            (reserve0, reserve1) = (bal0, bal1);
        }
    }

    ///@dev update the delta to reserve to keep it updated with balance
    ...
    function skim() external override lock {
        ...
        if (rebaseAmount.token0 > 0) {
            _token0 = rebaseAmount.token0;
            rebaseAmount.token0 = 0;
            TransferHelper.safeTransfer(token0, msg.sender, _token0);
        }
        if (rebaseAmount.token1 > 0) {
            _token1 = rebaseAmount.token1;
            rebaseAmount.token1 = 0;
            TransferHelper.safeTransfer(token1, msg.sender, _token1);
        }
