...
interface IPearlV2PoolFactory {
    ...
    /// @notice set pool rebase proxy address
    /// @param _pool address of the pool
    /// @param _rebaseProxy address of the pool rebase controller
    function setPoolRebaseProxy(address _pool, address _rebaseProxy) external;
    ...
    /// @notice Retrieves the manager address of a pool.
    /// @param pool The address of the pool contract.
    /// @return manager The address of the manager.
    function poolRebaseProxy(address pool) external view returns (address manager);

    /// @notice Checks if a proxy is allowed to skim a given pool.
    /// @param pool The address of the pool contract.
    /// @param proxy The address of the proxy contract.
    /// @return isAllowed True if the proxy is allowed, false otherwise.
    function isRebaseProxy(address pool, address proxy) external view returns (bool isAllowed);
}
